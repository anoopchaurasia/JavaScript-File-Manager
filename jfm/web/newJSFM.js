(function(win, isnode){
	...
	// /win.fm.import adds new javascript file to head of document. JAVA:
	// import
	...
	win.fm["package"] = win.fm.Package = function Package( packageName ) {
		scriptArr.pop();
		var script = {
	...
	win.fm['super'] = win.fm['base'] = win.fm.Base = function Base( baseClass ) {
		var script = scriptArr[scriptArr.length - 1];
	...
	win.fm["interface"] = win.fm.Interface = function Interface( ) {
		var script = scriptArr[scriptArr.length - 1];
		script.isInterface = true;
		this.Class.apply(this, arguments);
	};

	win.fm['abstractClass'] = win.fm.AbstractClass = function( ) {
		var script = scriptArr[scriptArr.length - 1];
	...
	win.fm['implements'] = win.fm.Implements = function Implements( ) {
	...
	// win.fm.Class creates a jfm class.
	win.fm['class'] = win.fm["Class"] = function Class( ){
	...
	function getAllImportClass( imp ) {
		...
		for ( var k = 0; imp && k < imp.length; k++) {
			splited = imp[k].split(".");
			newImports[splited[splited.length - 1]] = win.fm.stringToObject(imp[k]);
		}
		return newImports;
	}
	...
	function addPrototypeBeforeCall( Class, isAbstract ) {
		...
		saveState.push(win.Static, win.Abstract, win.Const, win.Private);
	...
	function defaultConstrct( ) {
		if (arguments.length > 0) {
			console.error("Class does not have any constructor ");
		}
	}
