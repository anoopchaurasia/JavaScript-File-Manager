...
(function (window, undefined) {
    ...
    //This method is adding a $add method into class prototype wchich is being used to create setter and getter for its own property.
    ...
    //currentScript is being used to contain all information of currently loaded JavaScript file.
    var currentScript;
    ...
    function fileReady(){
        ...
        ///  fm.Application can be assigned a function instead of calling it with function parameter.
        ...
        fm.holdReady = function (is) {
            if (is) {
                ...
                if (fm.isReaddyCounter == 0) {
                    /// incase onReadyFun is undefined then assuming fm.Application has been assigned a method. 
    ...
    //fm.stackTrace can be used to collect to print function stack; JAVA:Exception.stacktrace
    fm.stackTrace = function(message){
        ...
        // System.out.println(e.stack && e.stack.substring(e.stack.indexOf("\n")));
        }
    };
    ...
    ///onReadyState method get called when browser fail to load a javascript file.
    ...
    function include(path) { 
        ...
        //onerror is not supported by IE so this will throw exception only for non IE browsers.
    ...
    // Add all implemented interface to class interface list. and import them.JAVA:implements
    ...
    //callAfterDelay:Delay the call for classManager so that file get compiled completely. 
    //And classManager get all information about the function.
    function callAfterDelay(currentScript) {
        setTimeout(function () {
            //Calling classmanager after a short delay so that file get completely ready.
    ...
    var classDependent = {};
    // Call all callbacks after a class get ready so that dependent can complete.
    ...
    function checkMandSetF(pofn){
        ...
        pofn.prototype.$checkMAndGetF = function(pofnS, allMethods){
            var temp = {}, k, len;
            ...
            for(var k in baseClassObject){
                if(baseClassObject.hasOwnProperty(k)  ){
                    ...
                    if(typeof v == 'function'){
                        ...
                        v.$name = k;
                        v.$Class = cls;
                        v = changeContext( v, baseClassObject);
                        for(var l = arr.length - 1; l >= 0 ; l--){
                            if(arr[l][k] != undefined ) break;
                            arr[l][k] = v;
                        }
                    }else{
                        isConst && baseClassObject.$add(baseClassObject, k, v, isConst);
                        for(var m = arr.length - 1; m >= 0 ; m--){
                            if(arr[m][k] != undefined ) break;
                            baseClassObject.$add(arr[m], k, undefined, isConst);
                        }
                    }
                }
            }
            //deleteing $add as all operations on $add are completed for this instance.
            ...
            currentObj.setAbstractMethods = function(solidObj){
                for(var k in absMethods){
                    if(absMethods.hasOwnProperty(k)){
                        ...
                        this[k] = changeContext( solidObj[k], solidObj);
                    }
                }  
    ...
    function getReleventClassInfo( Class, fn, pofn){
        ...
        if(temp){  
            for( k = 0, len = temp.length; k < len;  k++ ){
                createObj(temp[k]).prototype.$checkMAndGetF(pofn, info.methods);              
            }
        }
        if(tempObj.init)tempObj.init();
    ...
    function getException( script, pofn){
        ...
        || ( pofn.prototype.$get("privateConstructor") && 
            ( caller.$Class != script.Class && caller.$Class != "jfm.io.Serialize")&&
    ...
    function createClassInstance(pofn, script, fn, Class){
        ...
        addPrototypeBeforeCall( Class, pofn.sAbstract );
    ...
    function executeOnready(script, fn, Class) {
        ...
        //for instance of: check if given class is a interface implemeted by host class.
        ...
        //script.isInterface && addFieldsInStorage.call(pofn, script, pofn, true);
    ...
    function createSetterGetterHelper(self, obj, source, isConst, isStatic){
        ...
        var isSameorCon = isConst || obj != self;
        for(var k in source){            
            if(source.hasOwnProperty(k)){
                ...
                if(typeof val == 'function'){
                    val.$name = k;
                    val.$Class = cls;
                    obj[k] == undefined && (obj[k] = changeContext( val, obj)); 
                }else
                if(isSameorCon){
    ...
    function classManager(script) {
        ...
        po[fn] = function manager() {
            ...
            !this.__base___ && currentObj.constructor.apply(currentObj, arguments); 
